	...
	<div id="container">
		...
		<div id="wrapper">
			<div id="content">
				...
				<c:forEach items="${pages.content }" var="reserve" varStatus="status">
					<tr>
						<td>${status.count }</td>
						<td>${reserve.rent.item.title }</td>
						<td>${reserve.rent.item.category.title }</td>
						<td>${reserve.rent.returnDate}</td>
						<td>${reserve.orderNo}번째</td>
					...
					<ul>
						<c:choose>
							<c:when test="${pager.prev }">
								<li style="color: black;"><a
									href="${ctx}/admin/reserve?page=${(pager.blockPage-1) * pager.pageSize}&kwd=${kwd}">◀</a></li>
							</c:when>
							<c:otherwise>
						...
						<c:forEach begin="${pager.pageStart }" end="${pager.pageStart + pager.pageSize - 1 }" var="page">  
							<c:choose>
								...
								<c:otherwise>
									<li><a href="${ctx }/admin/reserve?page=${page-1 }&kwd=${kwd}">${page }</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						...
						<c:choose>
							<c:when test="${pager.next }">
								<li style="color: black;"><a
									href="${ctx}/admin/reserve?page=${pager.blockPage * pager.pageSize + 1  }&kwd=${kwd}">▶</a></li>
							</c:when>
							<c:otherwise>
								<li style="color: #EAEAEA;">▶</li>
							</c:otherwise>
